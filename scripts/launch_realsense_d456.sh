...
ENABLE_IMU=false
RESOLUTION=848x480x15   # Default: max resolution @ 30 fps
...
ros2 launch realsense2_camera rs_launch.py rgb_camera.color_profile:=$RESOLUTION depth_module.depth_profile:=$RESOLUTION color_qos:=SENSOR_DATA depth_qos:=DEFAULT enable_sync:=true enable_gyro:=$ENABLE_IMU enable_accel:=$ENABLE_IMU align_depth.enable:=$USE_ALIGNED_DEPTH camera_name:=Front camera_namespace:=/MastCam/ & REALSENSE_PID=$!
